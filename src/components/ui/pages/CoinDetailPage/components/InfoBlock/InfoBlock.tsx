...
const InfoBlock:FC<PropsWithChildren<IInfoBlock>> = ({circulating_supply, market_cap, max_supply, current_price, total_volume, price_change_percentage_24h, price_change_24h}) => {
    return (
        <section className={styles.infoBlock}>
        <div>
            <p>Цена:</p>
            <div className={styles.priceContainer}> 
                <p className={price_change_percentage_24h > 0 ? styles.grPrice : styles.rPrice}>{current_price.toLocaleString()} </p>
        ...
        <div>
            ...
            <div className={styles.priceContainer}> 
                <p className={price_change_percentage_24h > 0 ? styles.grPrice : styles.rPrice}>{price_change_percentage_24h.toFixed(2)} %</p>
                <p className={price_change_percentage_24h > 0 ? styles.grPrice : styles.rPrice}>{price_change_24h.toFixed(2)} $</p>
            </div>
        </div>
        <div>
        ...
        <div>
            ...
            <p>{max_supply === null ? '- - - - - - - - - - - - - - - - -' : max_supply}</p>
        </div>
        </section>
    )
}
