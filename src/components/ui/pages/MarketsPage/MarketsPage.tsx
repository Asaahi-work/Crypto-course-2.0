            ...
            <div className={styles.market}>
                ...
                <section className={styles.instruments}>
                    ...
                    <div className={styles.coinlist}>
                        {
                            coinsQuery.isLoading ? <LoadingBox/> :
                            coinsQuery.data?.data ? coinsQuery.data.data.map((el:any) => (
                                <>
                                    <Coin id={el.id} key={el.id} symbol={el.symbol} name={el.name} image={el.image} current_price={el.current_price} market_cap={el.market_cap} price_change_24h={el.price_change_24h} price_change_percentage_24h={el.price_change_percentage_24h}/>
                                </>
                            )) : <div>No data found.</div>}
                    </div>
                </section>             
