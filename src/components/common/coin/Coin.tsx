...
const Coin: FC<PropsWithChildren<ICoinResponse>> = ({
  id,
  symbol,
  name,
  image,
    ...
    <Link to={`/coin/detail/${id}`}>
      <div className={styles.coin}>
        ...
        <p className={styles.priceChange}>
          ...
          >
            {price_change_24h.toFixed(2)} USD
          </span>
        </p>
        <p className={styles.priceChangeP}>
          Изменение за 24ч. в %{" "}
          <span
            className={
              price_change_percentage_24h > 0
        ...
        <p className={styles.marketCap}>
          Рыночная капитализация <span>{market_cap.toLocaleString()} </span>$
        </p>
      </div>
    </Link>
  )
